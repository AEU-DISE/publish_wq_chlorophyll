NOTE: this is for archival purposes and was created to try and get a workable import of the complete metadata for EDI but there's an error that I couldn't figure out. JA

---
title: "Interagency Ecological Program: Discrete water quality and phytoplankton data from the Sacramento
  River floodplain and Yolo Bypass tidal slough, collected by the Yolo Bypass Fish Monitoring
  Program, 1998 - 2022"
output:
  html_document:
    df_print: paged
...
```{r}
title <- "Interagency Ecological Program: Discrete water quality and phytoplankton data from the Sacramento River floodplain and Yolo Bypass tidal slough, collected by the Yolo Bypass Fish Monitoring Program, 1998 - 2022"
...
```{r}
keywords <- c("Yolo Bypass", "San Francisco Estuary", "Sacramento-San Joaquin Delta", "Yolo Bypass Fish Monitoring Program", "California Department of Water Resources", "Interagency Ecological Program", "tidal slough", "phytoplankton","water quality", "nutrients", "chlorophyll")
```
...
```{r}
tc <- set_coverage(beginDate = 1998,
                   endDate = 2022,
...
```{r}
wq <- read.csv("../data_clean/nutr_wq_clean.csv")
phyto <- read.csv("../data_clean/phyto_clean.csv")
```


```{r}
nutr_wq_atts <- read.csv("nutr_wq_atts.csv")%>% 
    select(-starts_with("X"))
nutr_wq_attlist <- set_attributes(nutr_wq_atts)
...
```{r}
nutr_wq_dt <- eml$dataTable(entityName = "../data_clean/nutr_wq_clean.csv",
                       entityDescription = "nutrient and water quality data",
                       attributeList = nutr_wq_attlist)
```


```{r}

phyto_atts <- read.csv("phyto_atts.csv")
phyto_attlist <- set_attributes(phyto_atts)
```
...
```{r}
phyto_dt <- eml$dataTable(entityName = "../data_clean/phyto_clean.csv",
                       entityDescription = "phytoplankton data",
                       attributeList = phyto_attlist)
```
...
doc <- list(
           ...
           dataset = eml$dataset(title = "Interagency Ecological Program: Discrete water quality and phytoplankton data from the Sacramento River floodplain and tidal slough, collected by the Yolo Bypass Fish Monitoring Program, 1998 - 2022",
                          abstract = ab,
                          creator = cres,
                          contact = cons,
                          associatedParty = aps,
                          ...
                          project = proj,
                          dataTable = list(phyto_dt, nutr_wq_dt)))

write_eml(doc, "WQLT_metadata.xml")
